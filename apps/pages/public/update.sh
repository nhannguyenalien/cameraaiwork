...
# Public copy of apps/relay/update.sh. Keep both files identical.
# Safe in-place update: no site claim and no replacement of .env/cameras.json.
...
if [[ -n "${CAMERAAIWORK_DIR:-}" ]]; then INSTALL_DIR="$CAMERAAIWORK_DIR"; else INSTALL_DIR="$DEFAULT_INSTALL_DIR"; fi
...
if [[ "${CAMERAAIWORK_UPDATE_SOURCE:-bundle}" == "git" && -d "$INSTALL_DIR/.git" ]] && command -v git >/dev/null 2>&1; then
  ...
  rsync -a --exclude='.git/' --exclude='.env' --exclude='.env.*' --exclude='cameras.json' --exclude='node_modules/' --exclude='.venv/' --exclude='infra/go2rtc/bin/' --exclude='infra/go2rtc/go2rtc.yaml' "$TEMP_DIR/release/" "$INSTALL_DIR/"
fi
...
case "$(uname -s)" in
  Darwin)
    ...
    sed -e "s|__UPDATE_SCRIPT__|$INSTALL_DIR/apps/relay/update.sh|g" "$INSTALL_DIR/infra/launchd/com.cameraaiwork.update.plist" > "$AUTO_PLIST"
    launchctl bootout "gui/$(id -u)/com.cameraaiwork.update" >/dev/null 2>&1 || true
    launchctl bootstrap "gui/$(id -u)" "$AUTO_PLIST"
    ;;
  Linux)
    sed -e "s|__INSTALL_DIR__|$INSTALL_DIR|g" "$INSTALL_DIR/infra/systemd/cameraaiwork-update.service" | sudo tee /etc/systemd/system/cameraaiwork-update.service >/dev/null
...
if [[ ! -x "$INSTALL_DIR/ai/worker/.venv/bin/python" ]]; then "$PYTHON_BIN" -m venv "$INSTALL_DIR/ai/worker/.venv"; fi
...
case "$(uname -s)" in
  Darwin)
    ...
    for label in com.cameraaiwork.ai com.cameraaiwork.relay; do
      if launchctl print "gui/$USER_ID/$label" >/dev/null 2>&1; then launchctl kickstart -k "gui/$USER_ID/$label"; else echo "Cảnh báo: chưa tìm thấy service $label; không restart service này." >&2; fi
    done
    ;;
  Linux) command -v systemctl >/dev/null 2>&1 || die "Thiếu systemctl."; sudo systemctl restart cameraaiwork-ai cameraaiwork-relay ;;
